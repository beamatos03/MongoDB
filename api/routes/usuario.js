...
const validaUsuario = [
    ...
    check('senha')
        ...
        }).withMessage('A senha informada não é segura. Informe no mínimo 1 caractére maiúsculo, 1 caractere minúscilo e 1 caractere especial'),
        check('ativo')
        .default(true)
        .isBoolean().withMessage('Deve ser um booleano, true ou false'),
        check('tipo').default('Cliente')
        .isIn(['Admin', 'Cliente']).withMessage('O tipo do usuario deve ser Admin ou Cliente'),
        check('avatar')
        .isURL().withMessage('O endereço do avatar deve ser URL válida')
]
